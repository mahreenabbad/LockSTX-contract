...
(define-public (unlock-stx
  ...
  (begin
    ...
    ;; 3. Check if it has already been unlocked
    (let ((existing-unlock (map-get? stx-unlocks { lock-id: lock-id })))
      (asserts! (is-none existing-unlock) ERR-UNLOCK-EXISTS)
      ...
      (let ((contract-balance (stx-get-balance (as-contract tx-sender))))
        ...
        (map-set stx-unlocks { lock-id: lock-id } {
          recipient: recipient,
          amount: amount,
          unlocked: true,
        })
...
(define-read-only (is-unlocked (lock-id uint))
  (let ((record (map-get? stx-unlocks { lock-id: lock-id })))
    (if (is-none record)
      (ok false)
      (ok (get unlocked (unwrap! record (err u0))))
    )
  )
)
...
(define-read-only (get-unlock-by-id (lock-id uint))
  (map-get? stx-unlocks { lock-id: lock-id })
)
